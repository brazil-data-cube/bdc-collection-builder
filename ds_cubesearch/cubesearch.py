...
app.logger_name = "cubesearch"
handler = logging.FileHandler('cubesearch.log')
...
def os_granule(output):
	...
	try:
		data = cubedata.get_bbox(request.args.get('bbox', None),
								  ...
								  start_index, count)
	except cubedata.InvalidBoundingBoxError:
	...
	resp = make_response(render_template('granule.{}'.format(output),
										 ...
										 data=data, total_results=total_results, start_index=start_index, count=count,
...
def os_dataset(output):
	...
	try:
		result = cubedata.get_datasets(request.args.get('bbox', None),
...
def os_osdd_granule():
	app.logger.warning('/osdd/granule')
	resp = make_response(render_template('osdd_granule.xml',
										 url=os.environ.get('BASE_URL'),
										 datasets=cubedata.get_datasets(),
										 bands=cubedata.get_bands(),
										 rps=cubedata.get_radiometricProcessing(),
										 types=cubedata.get_types()))
...
def browse_image(sceneid):
	try:
		image = cubedata.get_browse_image(sceneid)
	except IndexError:
...
def scene(sceneid):
	try:
		data, result_len = cubedata.get_bbox(uid=sceneid)
